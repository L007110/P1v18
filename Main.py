...
def rl(mean_loss_across_epochs=None, gnn_optimizer=None, device=None):
    ...
    while True:
        ...
        epoch_breakdown_stats = {
            'norm_snr': [], 'norm_delay': [], 'norm_v2i': [], 'norm_power': [],
            'raw_v2i': [], 'total_reward': []
        }
        ...
        if (USE_GNN_ENHANCEMENT and
            ...
            for experience in batch:
                ...
                # --- MODIFIED 1: 调用 forward 并获取 logits (不再需要 torch.no_grad()) ---
                # 移除 torch.no_grad() 意味着 GNN 模型现在是 Joint Training
                all_q_values_t, edge_attention_logits_t = global_gnn_model(graph_t_dev)
                with torch.no_grad():
                    all_q_values_t1_online, _ = global_gnn_model(graph_t1_dev)
                    all_q_values_t1_target, _ = global_target_gnn_model(graph_t1_dev)
                # --- END MODIFIED 1 ---

                # --- ADDED 2: 计算 Attention Entropy ---
                P = F.softmax(edge_attention_logits_t, dim=0)
                # 熵 H = - sum(P * log(P))
                ...
                if USE_GNN_ENHANCEMENT:
                    ...
                    if breakdown:
                        for k, v in breakdown.items():
                            if k in epoch_breakdown_stats:
                                epoch_breakdown_stats[k].append(v)
        ...
        avg_breakdown = {}
        for k, v_list in epoch_breakdown_stats.items():
            avg_breakdown[k] = np.mean(v_list) if v_list else 0.0

        # 打印详细诊断信息 (非常有用于观察 Grid Search 效果)
        debug_print(
            f"  [Reward Analysis] Norm Scores (0-1): "
            f"SNR={avg_breakdown['norm_snr']:.3f}, "
            f"Delay={avg_breakdown['norm_delay']:.3f}, "
            f"V2I={avg_breakdown['norm_v2i']:.3f}, "
            f"Power={avg_breakdown['norm_power']:.3f}"
        )
        debug_print(
            f"  [Raw Metrics] "
            f"V2I Penalty Power={avg_breakdown['raw_v2i']:.2e} W"
        )
...
def test():
    ...
    results_csv_path = f"{global_logger.log_dir}/scalability{Parameters.ABLATION_SUFFIX}.csv"
...
if __name__ == "__main__":
    ...
    # 2. P1v18 核心: 解析消融模式参数
    parser = argparse.ArgumentParser(description="P1v18 Final Reward Ablation")
    ...
    # 唯一的变量: 消融模式
    parser.add_argument("--ablation_mode", type=str, default="E0",
                        choices=["E0", "E1", "E2", "E3"],
                        help="E0:Full, E1:No-V2I, E2:No-Delay, E3:Only-SNR")
    ...
    Parameters.ABLATION_MODE = args.ablation_mode

    # 更新后缀 (这是区分实验结果的关键)
    Parameters.ABLATION_SUFFIX = f"_P1v18_{args.ablation_mode}"

    # 打印配置确认
    debug_print("=" * 40)
    debug_print(f"       P1v18 FINAL ABLATION")
    debug_print("=" * 40)
    debug_print(f"Mode       : {Parameters.ABLATION_MODE}")
    debug_print(f"Optimized Weights (Fixed):")
    debug_print(f"  SNR={Parameters.SNR_MULTIPLIER}")
    debug_print(f"  V2I={Parameters.V2I_MULTIPLIER}")
    debug_print(f"  DLY={Parameters.DELAY_MULTIPLIER}")
    debug_print(f"Suffix     : {Parameters.ABLATION_SUFFIX}")
    debug_print("=" * 40)
    ...
    # 5. 显示模型信息
    if USE_UMI_NLOS_MODEL:
        debug_print("Channel: UMi NLOS with NewRewardCalculator (Optimized)")
    else:
        debug_print("Channel: Original Baseline")
    ...
    if Parameters.RUN_MODE == "TRAIN":
        ...
        formulate_global_list_dqn(global_dqn_list, device)
        for dqn in global_dqn_list:
            debug_print(f"Init Agent: {dqn.dqn_id}")

        # 优化器防御初始化
        gnn_optimizer = None

        if USE_GNN_ENHANCEMENT:
            debug_print("GNN Enhancement: ENABLED")
            global_gnn_model.to(device)
            global_target_gnn_model.to(device)
            gnn_optimizer = optim.Adam(global_gnn_model.parameters(), lr=RL_ALPHA_GNN)
        else:
            debug_print("GNN Enhancement: DISABLED")

        # 传入 device，防止报错
        rl(gnn_optimizer=gnn_optimizer, device=device)

    elif Parameters.RUN_MODE == "TEST":
        debug_print("========== STARTING TEST MODE ==========")
        test()

    else:
        debug_print(f"!!! Error: Unknown RUN_MODE '{Parameters.RUN_MODE}'")